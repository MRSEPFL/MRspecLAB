...
def loadInput(self):
    ...
    self.originalData = []
    self.header = None
    self.issvs = True # check svs or multi-voxel data
    vendor = None
    dtype = None
    ...
    for filepath in self.filepaths:
        try: data, header, dtype, vendor = load_file(filepath)
            ...
            elif len(data.shape) > 1: # data is multi-dimensional
        ...
        for k, v in utils.supported_sequences.items():
            for seq in v:
                if seq in seqstr:
                    self.sequence = k
                    break
        if self.sequence is None: utils.log_warning("Sequence not in default: " + seqstr)
...
def processStep(self, step, nstep):
    ...
    if self.save_raw_button.GetValue():
        ...
        output_np = np.array(dataDict["output"])
        utils.log_info(f"Shape of the dataDict_output: {output_np.shape}")

        for i, d in enumerate(dataDict["output"][0]): 
            save_raw(os.path.join(filepath, str(i) + ".RAW"), d, seq=self.sequence)
        if self.originalWref is not None: 
            for i, d in enumerate(dataDict["wref_output"][0]): 
        ...
        for key in ["Nucleus", "nucleus"]:
            if key in self.header.keys():
                if self.header[key] != "1H": 
                    ...
                    utils.log_info(f"Running LCModel for {workpath} {label}...")
                    utils.log_debug("\n\t", command)
                    subprocess.run(command, shell=True)
