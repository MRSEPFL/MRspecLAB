...
from gsnodegraph.gsnodegraph.node import NodeWire
...
class NodeGraphBase(wx.ScrolledCanvas):
    ...
    def OnDeleteNode(self, event):
        if (self.active_node != None and
            self.active_node.IsOutputNode() != True and self.active_node.IsInputNode() != True): ##Changed for MRS
    ...
    def CreateContextMenu(self):
        ...
        if self.active_node != None:
            ...
            if (self.active_node.IsOutputNode() != True and self.active_node.IsInputNode() != True):
    ...
    def DeleteNodes(self):
        ...
        """
        for node in self.sel_nodes:
            if (node.IsOutputNode() != True and node.IsInputNode() != True):##Changed for MRS
        ...
        if (self.active_node != None and
            self.active_node.IsOutputNode() != True and node.IsInputNode() != True):##Changed for MRS
            self.DeleteNode(self.active_node)
            self.active_node = None
    ...
    def DuplicateNode(self, node):
        ...
        """
        if (node.IsOutputNode() is not True and node.IsInputNode() is not True): #changed for MRS softfare
