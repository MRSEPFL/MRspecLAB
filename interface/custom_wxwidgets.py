...
#This code as been modified such that there is no import from the gswidgetkit repository

##function also taken from gswidgetkit repository
...
##Image also took from gswidgetkit repository
...
##Following lines of codes added to make the color of the widget correspond to the general colorscheme 
from constants import(BLACK_WX,GREY_WX,ORANGE_WX,BEIGE_WX)
ACCENT_COLOR=ORANGE_WX
TEXT_COLOR=BEIGE_WX
DROPDOWN_BG_COLOR =BLACK_WX
...
class DropDown(wx.Control):
    ...
    def AddMenuItem(self,itemi): ##added for MRS software

        nb_of_items=len(self.dropdown_menu.GetMenuItems())
        print(nb_of_items)
        i=nb_of_items+1
        # for item in self.dropdown_menu.GetMenuItems():
        #     menu_item = flatmenu.FlatMenuItem(self.dropdown_menu,
        #                                       DROPDOWNMENU_ITEM_IDS[i],
        #                                       item, "", wx.ITEM_NORMAL)
        #     self.dropdown_menu.AppendItem(menu_item)
        ...
        menu_item = flatmenu.FlatMenuItem(self.dropdown_menu,
                                            DROPDOWNMENU_ITEM_IDS[nb_of_items],
                                            itemi, "", wx.ITEM_NORMAL)
        self.dropdown_menu.AppendItem(menu_item)
        self.dropdown_menu.Append(DROPDOWNMENU_ITEM_IDS[nb_of_items],itemi, "", wx.ITEM_NORMAL)
        # self.dropdown_menu.AddItem(menu_item)


        self.menuidmapping[DROPDOWNMENU_ITEM_IDS[nb_of_items]] = itemi

        self.Bind(wx.EVT_MENU, self.OnSelectMenuItem, id=DROPDOWNMENU_ITEM_IDS[nb_of_items])

        self.dropdown_menu.Refresh()
